  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      try {
        ...
        // Create the doctor data object with field names matching backend expectations
        const doctorData = {
          ...
          medicalLicense: formData.license, // Match backend field name
          age: age,
          gender: formData.gender,
          phoneNumber: formData.countryCode + formData.phone, // Backend expects "phoneNumber" not "phone"
          location: formData.address, // Backend expects "location" not "address"
          profileImage: profileImage ? profileImage.split(',')[1] : null,
          description: formData.description, // Backend might expect "description" instead of "about"
        }

        console.log("Sending doctor data:", doctorData);
        
        // Call the API endpoint directly with fetch to ensure correct path
        const response = await fetch('http://localhost:8081/api/doctors/signup', {
          method: 'POST',
          headers: {
